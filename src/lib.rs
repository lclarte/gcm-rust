...
use gcmrust::state_evolution::state_evolution::StateEvolution;
use gcmrust::utility;
use pyo3::prelude::*;
...
#[pyfunction]
fn pseudo_bayes_logistic_z0(y : f64, w : f64, v : f64, beta : f64, normalized : bool) -> f64 {
    if normalized {
        let student_partition = channels::normalized_pseudo_bayes_logistic::NormalizedPseudoBayesLogistic {
            beta  : beta
        };
        return student_partition.z0(y, w, v);
    }
    else {
        let student_partition = channels::pseudo_bayes_logistic::PseudoBayesLogistic {
            beta  : beta
        };
        return student_partition.z0(y, w, v);
    }
...
#[pyfunction]
fn logistic_z0(y : f64, w : f64, v : f64) -> f64 {
    let partition = data_models::logit::Logit {
        noise_variance  : 0.0
    };
    return partition.z0(y, w, v);
}
...
fn gcmpyo3(_py: Python<'_>, m: &PyModule) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(pseudo_bayes_logistic_z0, m)?)?;
    m.add_function(wrap_pyfunction!(logistic_z0, m)?)?;
